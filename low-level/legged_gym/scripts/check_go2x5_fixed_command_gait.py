...
def evaluate_case(env, policy, name, vx, yaw, cli):
    ...
    if translation_ratio is not None:
        gait_checks.append(translation_ratio >= cli.min_translation_progress_ratio)
    if yaw_ratio is not None:
        gait_checks.append(yaw_ratio >= cli.min_yaw_progress_ratio)
    if swing_contact_fraction is not None:
        gait_checks.append(swing_contact_fraction <= cli.max_swing_contact_fraction)
    if swing_height_mean is not None:
        gait_checks.append(swing_height_mean >= cli.min_swing_height)
    behavior_passed = all(gait_checks)
