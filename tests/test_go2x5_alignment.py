...
def test_configs_do_not_fall_back_to_old_go2x5_names():
    ...
    assert 'profile_name = "go2x5_flat_tabletop_6d_walk_v11"' in go2x5_config
    ...
    assert "feet_height_target = 0.05" in go2x5_config
    assert "standing_probability = 0.10" in go2x5_config
    assert "straight_line_probability = 0.40" in go2x5_config
    ...
    assert "tracking_ang_vel = 1.0" in go2x5_config
...
def test_go2x5_simple_training_design_matches_current_plan():
    ...
    assert "feet_height = 1.0" in go2x5_config
    ...
    assert "leg_action_l2_deadzone = -0.5" in go2x5_config
    assert "leg_action_deadzone = 0.80" in go2x5_config
    assert "tracking_ee_world_stable = 0.0" in go2x5_config
